...
class Create_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		$floor = intval($_GPC['floor']);	
		if (empty($floor) || $floor < 1) {
			$floor = 1;
		}
		...
		if (!empty($gift['giftgoodsid'])) {
			...
			if ($giftGoodsid) {
				foreach ($giftGoodsid as $key => $value) {
					$giftGood[$key] = pdo_fetch('select id,title,thumb,marketprice,handlingfee from ' . tablename('ewei_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
				}
			}
		}
		...
		if (!$packageid) {
			...
			if ($giftid) {
				...
				if ($giftdata['giftgoodsid']) {
					...
					foreach ($giftgoodsid as $key => $value) {
						$gift[$key] = pdo_fetch('select id as goodsid,title,thumb, handlingfee from ' . tablename('ewei_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
				...
				foreach ($gifts as $key => $value) {
					...
					foreach ($giftgoods as $k => $val) {
						$gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice,handlingfee from ' . tablename('ewei_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $val . ' ');
					}
			...
			foreach ($g as $key => $value) {
				$goods[$key] = pdo_fetch('select id,title,thumb,marketprice,handlingfee from ' . tablename('ewei_shop_goods') . "\r\n                            where id = " . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
				$option = array();
				$packagegoods = array();
	...
	public function caculate()
	{
		...
		if (is_array($goodsarr)) {
			...
			foreach ($goodsarr as &$g) {
				...
				if (!empty($optionid)) {
					$option = pdo_fetch('select id,title,marketprice,handlingfee, goodssn,productsn,stock,`virtual`,weight from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
				...
				$data['handlingfee'] = $prices['handlingfee'];
				...
				$totalhandlingfee += $data['handlingfee'];
				$allgoods[] = $data;
				if ((0 < floatval($g['buyagain'])) && empty($g['buyagain_sale'])) {
					if (m('goods')->canBuyAgain($g)) {
						$buyagain_sale = false;
		...
		$return_array['totalhandlingfee'] = $totalhandlingfee;
		$return_array['floor'] = $floor;
	...
	public function submit()
	{
		...
		$order['handlingfee'] = $totalhandlingfee;
		$order['floor'] = $floor;
		$order['floor'] = $floor;
		...
		if ($multiple_order == 0) {
			...
			foreach ($merch_array as $key => $value) {
				...
				$order['handlingfee'] = $ch_order_data[$merchid]['totalhandlingfee'];
				$order['floor'] = $floor;
	...
	protected function singleDiyformData($id = 0)
	{
		...
		if ($diyform_plugin && !empty($id)) {
			$sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode,diyfields,handlingfee FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';

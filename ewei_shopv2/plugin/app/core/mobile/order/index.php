...
class Index_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		foreach ($list as &$row) {
			...
			if($iscreditgoods){
				$sql = 'SELECT og.goodsid,og.total,g.title,g.thumb,og.price FROM ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_creditshop_goods') . ' g on og.goodsid = g.id ' . ' where ' . $scondition . ' order by og.id asc';
			}
			...
			$row['cancancel'] = !$iscreditgoods && !$row['userdeleted'] && !$row['status'];
	...
	public function detail()
	{
		...
		$iscreditgoods = false;
		if(preg_match('/^DH.*/', $order['ordersn'])){
			$iscreditgoods = true;
		}
		...
		$sql = 'select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids' . $diyformfields . '  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ';

		if($iscreditgoods){
			$sql = 'select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit, g.isverify,g.storeids' . $diyformfields . '  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_creditshop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ';
		}
		$goods = pdo_fetchall($sql, $param);
		...
		foreach ($goods as $g) {
			...
			$allgoods[] = array('id' => $g['goodsid'], 'title' => $g['title'], 'price' => $g['price'], 'thumb' => tomedia($g['thumb']), 'total' => $g['total'], 'iscreditgoods' => $g['iscreditgoods'], 'optionname' => $g['optiontitle'], 'diyformdata' => empty($g['diyformdata']) ? array() : $g['diyformdata'], 'diyformfields' => $newFields);
		}
